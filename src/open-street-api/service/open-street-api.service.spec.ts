...
describe('OpenStreetService', () => {
  ...
  beforeEach(async () => {
    ...
    openStreetApiService =
      module.get<OpenStreetApiService>(OpenStreetApiService);
  });

  it('should be defined', () => {
    expect(openStreetApiService).toBeDefined();
  });

  it('getGeoJSONFeatures response', async () => {
    expect(
      await openStreetApiService.getGeoJSONFeatures('0,90,60,90'),
    ).toStrictEqual(getGeoJSONResponse);
  });
});
